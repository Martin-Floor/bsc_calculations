...
def jobArrays(jobs, script_name=None, job_name=None, ntasks=1, gpus=1 ,mem_per_cpu=None, highmem=False,
              partition='bsc_ls', cpus_per_task=None, output=None, mail=None, time=48, module_purge=False,
              modules=None, conda_env=None, unload_modules=None, program=None, conda_eval_bash=False, account='bsc72',
    ...
    with open(script_name,'w') as sf:
        ...
        if program == 'gromacs':
            sf.write('export SLURM_CPU_BIND=none\n')
            sf.write('export OMP_NUM_THREADS=$SLURM_CPUS_PER_TASK\n')
            sf.write('export GMX_ENABLE_DIRECT_GPU_COMM=1\n')
            sf.write('export GMX_GPU_PME_DECOMPOSITION=1\n')
            sf.write('GMXBIN="mpirun --bind-to none -report-bindings gmx_mpi"\n')
...
def singleJob(job, script_name=None, job_name=None, cpus=112, mem_per_cpu=None, highmem=False,
              partition=None, threads=None, output=None, mail=None, time=None, pythonpath=None, account='bsc72',
    ...
    if program == 'pele':
        if modules == None:
            modules = []
        modules += modules+['intel', 'impi', 'mkl', 'cmake', 'boost' ,'anaconda', 'bsc', 'transfer']
